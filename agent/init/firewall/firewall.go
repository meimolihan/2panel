...
import (
	...
	firewallClient "github.com/2Panel-dev/2Panel/agent/utils/firewall/client"
	"github.com/2Panel-dev/2Panel/agent/utils/firewall/client/iptables"
)
...
func Init() {
	...
	if len(panelPort) == 0 {
		global.LOG.Errorf("find 2panel service port failed")
		return
	}
...
func needInit() bool {
	file, err := os.OpenFile("/run/2panel_boot_mark", os.O_RDWR|os.O_CREATE|os.O_EXCL, 0644)
	if err != nil {
		if os.IsExist(err) {
			return false
		}
