...
import (
	...
	"github.com/2Panel-dev/2Panel/agent/utils/toolbox/helper"
)
...
func NewFtpClient() (*Ftp, error) {
	...
	if err == nil {
		if err := cmd.NewCommandMgr().Run("useradd", "-u", "1000", "-g", groupItem.Name, "2panel"); err != nil {
			return nil, err
		}
		return &Ftp{DefaultUser: "2panel", DefaultGroup: groupItem.Name}, nil
	}
	if err.Error() != user.UnknownGroupIdError("1000").Error() {
		return nil, err
	}
	if err := cmd.NewCommandMgr().Run("groupadd", "-g", "1000", "2panel"); err != nil {
		return nil, err
	}
	if err := cmd.NewCommandMgr().Run("useradd", "-u", "1000", "-g", "2panel", "2panel"); err != nil {
		return nil, err
	}
	return &Ftp{DefaultUser: "2panel", DefaultGroup: "2panel"}, nil
}
