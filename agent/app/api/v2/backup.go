...
// @Accept json
// @Param request body dto.FullBackupRequest true "request"
// @Success 200
// @Security ApiKeyAuth
// @Security Timestamp
// @Router /backups/fullbackup [post]
// @x-panel-log {"bodyKeys":[],"paramKeys":[],"BeforeFunctions":[],"formatZH":"创建全量备份","formatEN":"Create full backup"}
func (b *BaseApi) CreateFullBackup(c *gin.Context) {
	var req dto.FullBackupRequest
	if err := helper.CheckBindAndValidate(&req, c); err != nil {
		return
	}
	info, err := fullBackupService.Create(req)
	if err != nil {
		helper.InternalServer(c, err)
		return
	}
...
// @Tags Backup Account
// @Summary Restore full backup
// @Accept json
// @Param request body dto.FullRecoverRequest true "request"
// @Success 200
// @Security ApiKeyAuth
// @Security Timestamp
// @Router /backups/fullrecover [post]
// @x-panel-log {"bodyKeys":["id"],"paramKeys":[],"BeforeFunctions":[{"input_column":"id","input_value":"id","isList":false,"db":"backup_records","output_column":"name","output_value":"name"}],"formatZH":"从全量备份 [name] 恢复","formatEN":"Restore from full backup [name]"}
func (b *BaseApi) RestoreFullBackup(c *gin.Context) {
	var req dto.FullRecoverRequest
	if err := helper.CheckBindAndValidate(&req, c); err != nil {
		return
	}
	if err := fullBackupService.Restore(req); err != nil {
		helper.InternalServer(c, err)
		return
	}
	helper.Success(c)
...
// @Tags Backup Account
// @Summary Search full backup records
// @Accept json
// @Param request body dto.PageSnapshot true "request"
// @Success 200 {object} dto.PageResult
// @Security ApiKeyAuth
// @Security Timestamp
// @Router /backups/fullbackup/search [post]
func (b *BaseApi) SearchFullBackup(c *gin.Context) {
	var req dto.PageSnapshot
	if err := helper.CheckBindAndValidate(&req, c); err != nil {
		return
	}
	total, items, err := fullBackupService.SearchWithPage(req)
	if err != nil {
		helper.InternalServer(c, err)
		return
	}
...
// @Tags Backup Account
// @Summary Delete full backup
// @Accept json
// @Param request body dto.SnapshotBatchDelete true "request"
// @Success 200
// @Security ApiKeyAuth
// @Security Timestamp
// @Router /backups/fullbackup/del [post]
// @x-panel-log {"bodyKeys":["ids"],"paramKeys":[],"BeforeFunctions":[{"input_column":"id","input_value":"ids","isList":true,"db":"backup_records","output_column":"file_name","output_value":"files"}],"formatZH":"删除全量备份 [files]","formatEN":"Delete full backup [files]"}
func (b *BaseApi) DeleteFullBackup(c *gin.Context) {
	var req dto.SnapshotBatchDelete
	if err := helper.CheckBindAndValidate(&req, c); err != nil {
		return
	}
	for _, id := range req.Ids {
		if err := fullBackupService.Delete(id, req.DeleteWithFile); err != nil {
			helper.InternalServer(c, err)
			return
		}
	}
...
// @Tags Backup Account
// @Summary Update full backup description
// @Accept json
// @Param request body dto.UpdateDescription true "request"
// @Success 200
// @Security ApiKeyAuth
// @Security Timestamp
// @Router /backups/fullbackup/description/update [post]
// @x-panel-log {"bodyKeys":["id","description"],"paramKeys":[],"BeforeFunctions":[{"input_column":"id","input_value":"id","isList":false,"db":"backup_records","output_column":"file_name","output_value":"files"}],"formatZH":"全量备份描述信息修改 [description]","formatEN":"Full backup description update [description]"}
func (b *BaseApi) UpdateFullBackupDescription(c *gin.Context) {
	var req dto.UpdateDescription
	if err := helper.CheckBindAndValidate(&req, c); err != nil {
		return
	}
	if err := fullBackupService.UpdateDescription(req); err != nil {
		helper.InternalServer(c, err)
		return
	}
	helper.Success(c)
