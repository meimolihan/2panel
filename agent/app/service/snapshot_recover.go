...
var (
	...
	svcCoreName, _     = controller.LoadServiceName("2panel-core")
	selCoreName, _     = controller.SelectInitScript("2panel-core")
	scriptCoreName, _  = controller.GetScriptName("2panel-core")
	svcAgentName, _    = controller.LoadServiceName("2panel-agent")
	selAgentName, _    = controller.SelectInitScript("2panel-agent")
	scriptAgentName, _ = controller.GetScriptName("2panel-agent")
)
	...
	go func() {
		...
		if req.IsNew || snap.InterruptStep == "RecoverBackups" {
			taskItem.AddSubTaskWithAliasAndOps(
				...
				func(t *task.Task) error {
					...
					err := itemHelper.FileOp.TarGzExtractPro(path.Join(rootDir, snap.Name, "/2panel_backup.tar.gz"), snapJson.BackupDataDir, "")
					itemHelper.Task.LogWithStatus(i18n.GetMsgByKey("Decompress"), err)
					return err
				},
				nil, 0, 90*time.Minute,
		...
		if req.IsNew || snap.InterruptStep == "RecoverPanelData" {
			taskItem.AddSubTaskWithAliasAndOps(
				...
				func(t *task.Task) error {
					...
					err := itemHelper.FileOp.TarGzExtractPro(path.Join(rootDir, snap.Name, "/2panel_data.tar.gz"), path.Join(snapJson.BaseDir, "2panel"), "")
					itemHelper.Task.LogWithStatus(i18n.GetMsgByKey("Decompress"), err)
					if err != nil {
						return err
					}
		...
		if req.IsNew || snap.InterruptStep == "RecoverCompose" {
			taskItem.AddSubTaskWithAliasAndOps(
				"RecoverCompose",
				func(t *task.Task) error {
					return restartCompose(path.Join(snapJson.BaseDir, "2panel/docker/compose"), &itemHelper)
				},
				nil, 0, 90*time.Minute,
			)
			req.IsNew = true
...
func backupBeforeRecover(name string, itemHelper *snapRecoverHelper) error {
	...
	rootDir := fmt.Sprintf("%s/2panel_original/original_%s", global.Dir.BaseDir, name)
	...
	if global.IsMaster {
		err = itemHelper.FileOp.CopyFile("/usr/local/bin/2pctl", baseDir)
		itemHelper.Task.LogWithStatus(i18n.GetWithName("SnapCopy", "/usr/local/bin/2pctl"), err)
		if err != nil {
			return err
		}
		err = itemHelper.FileOp.CopyFile("/usr/local/bin/2panel-core", baseDir)
		itemHelper.Task.LogWithStatus(i18n.GetWithName("SnapCopy", "/usr/local/bin/2panel-core"), err)
		if err != nil {
			return err
		}
		svcCorePath, _ := controller.GetServicePath("2panel-core")
	...
	err = itemHelper.FileOp.CopyFile("/usr/local/bin/2panel-agent", baseDir)
	itemHelper.Task.LogWithStatus(i18n.GetWithName("SnapCopy", "/usr/local/bin/2panel-agent"), err)
	if err != nil {
		return err
	}
	svcAgentPath, _ := controller.GetServicePath("2panel-agent")
...
func recoverBaseData(src string, itemHelper *snapRecoverHelper) error {
	...
	if global.IsMaster {
		err := itemHelper.FileOp.CopyFile(path.Join(src, "2pctl"), "/usr/local/bin")
		itemHelper.Task.LogWithStatus(i18n.GetWithName("SnapCopy", "/usr/local/bin/2pctl"), err)
		if err != nil {
			return err
		}
		err = itemHelper.FileOp.CopyFile(path.Join(src, "2panel-core"), "/usr/local/bin")
		itemHelper.Task.LogWithStatus(i18n.GetWithName("SnapCopy", "/usr/local/bin/2panel-core"), err)
		if err != nil {
			return err
		}
	...
	err := itemHelper.FileOp.CopyFile(path.Join(src, "2panel-agent"), "/usr/local/bin")
	itemHelper.Task.LogWithStatus(i18n.GetWithName("SnapCopy", "/usr/local/bin/2panel-agent"), err)
	if err != nil {
		return err
	}
