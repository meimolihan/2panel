...
func (u *SnapshotService) SnapshotRollback(req dto.SnapshotRecover) error {
	...
	go func() {
		rootDir := fmt.Sprintf("%s/2panel_original/original_%s", global.Dir.BaseDir, snap.Name)
		baseDir := path.Join(rootDir, "base")

		FileOp := files.NewFileOp()
		taskItem.AddSubTask(
			i18n.GetWithName("SnapCopy", "/usr/local/bin/2pctl"),
			func(t *task.Task) error {
				return FileOp.CopyFile(path.Join(baseDir, "2pctl"), "/usr/local/bin")
			},
			nil,
		)
		if global.IsMaster {
			taskItem.AddSubTask(
				i18n.GetWithName("SnapCopy", "/usr/local/bin/2panel-core"),
				func(t *task.Task) error {
					return FileOp.CopyFile(path.Join(baseDir, "2panel"), "/usr/local/bin")
				},
				nil,
			)
		}
		taskItem.AddSubTask(
			i18n.GetWithName("SnapCopy", "/usr/local/bin/2panel-agent"),
			func(t *task.Task) error {
				return FileOp.CopyFile(path.Join(baseDir, "2panel-agent"), "/usr/local/bin")
			},
			nil,
		)
		if global.IsMaster {
		...
		taskItem.AddSubTask(
			...
			func(t *task.Task) error {
				return FileOp.CopyDir(path.Join(rootDir, "2panel"), global.Dir.BaseDir)
			},
			nil,
		)
