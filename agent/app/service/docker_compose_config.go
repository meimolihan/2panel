...
type composeExposedPort struct {
	ContainerPort int
	HostPort      int
	HostIP        string
	Protocol      string
...
func loadComposeExposedPortsFromEnv(envs map[string]string, defaultHostIP string, strict bool) ([]composeExposedPort, error) {
	var ports []composeExposedPort
	for key, value := range envs {
		...
		ports = append(ports, composeExposedPort{
