...
import (
	...
	"github.com/2Panel-dev/2Panel/agent/utils/convert"
	"github.com/2Panel-dev/2Panel/agent/utils/ini_conf"
...
func (f *FileService) Delete(op request.FileDelete) error {
	if op.IsDir {
		...
		if path.Base(op.Path) == ".2panel_clash" || op.Path == excludeDir {
...
func (f *FileService) AISearch(req request.FileAISearch) (*response.FileAISearchResult, error) {
	...
	_ = searchOpts.ContentMatchDescription()
