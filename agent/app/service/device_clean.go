...
import (
	...
	fileUtils "github.com/2Panel-dev/2Panel/agent/utils/files"
...
const (
	rollbackPath = "2panel/tmp"
	upgradePath  = "2panel/tmp/upgrade"
	uploadPath   = "2panel/uploads"
	downloadPath = "2panel/download"
)
...
func (u *DeviceService) Scan() dto.CleanData {
	...
	originalPath := path.Join(global.Dir.BaseDir, "2panel_original")
	originalSize, _ := fileOp.GetDirSize(originalPath)
	treeData = append(treeData, dto.CleanTree{
		ID:          uuid.NewString(),
		Label:       "2panel_original",
		Size:        uint64(originalSize),
		IsCheck:     originalSize > 0,
		IsRecommend: true,
		CanDelete:   false,
		Type:        "2panel_original",
		Children:    loadTreeWithDir(true, "2panel_original", originalPath, fileOp),
	})
...
func (u *DeviceService) Clean(req []dto.Clean) {
	...
	for _, item := range req {
		...
		case "2panel_original":
			dropFileOrDir(path.Join(global.Dir.BaseDir, "2panel_original", item.Name))

		case "upgrade":
			dropFileOrDir(path.Join(global.Dir.BaseDir, upgradePath, item.Name))

		case "agent":
			dropFileOrDir(path.Join(global.Dir.BaseDir, "2panel/agent/package", item.Name))
...
func doSystemClean(taskItem *task.Task) func(t *task.Task) error {
	return func(t *task.Task) error {
		size := int64(0)
		fileCount := 0
		dropWithTask(path.Join(global.Dir.BaseDir, "2panel_original"), taskItem, &size, &fileCount)
...
func loadAgentPackage(fileOp fileUtils.FileOp) dto.CleanTree {
	pathItem := path.Join(global.Dir.BaseDir, "2panel/agent/package")
...
func dropTaskLog(logDir string) {
	...
	case "Tamper":
		xpackDB, err := common.LoadDBConnByPathWithErr(path.Join(global.CONF.Base.InstallDir, "2panel/db/xpack.db"), "xpack.db")
		if err == nil {
