...
import (
	...
	"github.com/2Panel-dev/2Panel/agent/utils/docker"
...
func snapBaseData(snap snapHelper, targetDir string, withDockerConf bool) error {
	...
	if global.IsMaster {
		err := snap.FileOp.CopyFile("/usr/local/bin/2panel-core", targetDir)
		snap.Task.LogWithStatus(i18n.GetWithName("SnapCopy", "/usr/local/bin/2panel-core"), err)
		if err != nil {
			return err
		}
		err = snap.FileOp.CopyFile("/usr/local/bin/2pctl", targetDir)
		snap.Task.LogWithStatus(i18n.GetWithName("SnapCopy", "/usr/local/bin/2pctl"), err)
		if err != nil {
			return err
		}
	}

	err := snap.FileOp.CopyFile("/usr/local/bin/2panel-agent", targetDir)
	snap.Task.LogWithStatus(i18n.GetWithName("SnapCopy", "/usr/local/bin/2panel-agent"), err)
	if err != nil {
		return err
	}
	...
	svcAgentName, _ := controller.GetServicePath("2panel-agent")
...
func snapBackupData(snap snapHelper, req dto.SnapshotCreate, targetDir string) error {
	...
	err := snap.FileOp.TarGzCompressPro(false, global.Dir.LocalBackupDir, path.Join(targetDir, "2panel_backup.tar.gz"), "", strings.Join(excludes, ","))
...
func snapPanelData(snap snapHelper, req dto.SnapshotCreate, targetDir string) error {
	...
	excludes = append(excludes, req.IgnoreFiles...)
	err := snap.FileOp.TarGzCompressPro(false, rootDir, path.Join(targetDir, "2panel_data.tar.gz"), "", strings.Join(excludes, ","))
...
func loadPanelExcludes(req []dto.DataTree) []string {
	...
	for _, item := range req {
		if len(item.Children) == 0 {
			if !item.IsCheck {
				itemDir, _ := filepath.Rel(path.Join(global.Dir.BaseDir, "2panel"), item.Path)
				excludes = append(excludes, itemDir)
			}
		} else {
