...
import (
	v2 "github.com/2Panel-dev/2Panel/agent/app/api/v2"
...
func (s *BackupRouter) InitRouter(Router *gin.RouterGroup) {
	...
	baseApi := v2.ApiGroupApp.BaseApi
	{
		...
		backupRouter.POST("/fullbackup", baseApi.CreateFullBackup)
		backupRouter.POST("/fullbackup/search", baseApi.SearchFullBackup)
		backupRouter.POST("/fullbackup/del", baseApi.DeleteFullBackup)
		backupRouter.POST("/fullbackup/description/update", baseApi.UpdateFullBackupDescription)
		backupRouter.POST("/fullrecover", baseApi.RestoreFullBackup)
	}
}
