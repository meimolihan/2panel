...
info "写入发版备注 RELEASE_NOTES.md"
{
  ...
  printf 'docker pull mobufan/2panel:latest\n'
  printf '\n'
  printf '```bash\n'
  printf 'docker pull mobufan/2panel:%s\n' "${TAG}"
  printf '```\n'
  printf '\n'
  printf '```bash\n'
  printf 'docker pull ghcr.io/meimolihan/2panel:latest\n'
  printf '```\n'
  printf '\n'
  printf '```bash\n'
  printf 'docker pull ghcr.io/meimolihan/2panel:%s\n' "${TAG}"
  printf '```\n'
  printf '\n'
  printf '## 二进制安装\n'
  printf '```bash\n'
  printf 'bash -c "$(curl -sSL https://raw.githubusercontent.com/meimolihan/2Panel/main/scripts/install.sh)" -p 8080 -d /var/lib/2panel\n'
  printf '```\n'
  printf '\n'
  printf '## 二进制卸载\n'
  printf '```bash\n'
  printf 'bash -c "$(curl -sSL https://raw.githubusercontent.com/meimolihan/2Panel/main/scripts/uninstall.sh)" -y --purge\n'
  printf '```\n'
} > RELEASE_NOTES.md
...
echo -e "${gl_bai}远程安装命令： ${gl_lv}bash -c \"\$(curl -sSL https://raw.githubusercontent.com/meimolihan/2Panel/main/scripts/install.sh)\" -p 8080 -d /var/lib/2panel${gl_bai}"
