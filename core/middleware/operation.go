...
import (
	...
	psessionUtils "github.com/2Panel-dev/2Panel/core/init/session/psession"
...
func newDB(pathItem string) (*gorm.DB, error) {
	...
	case strings.HasPrefix(pathItem, "/core/xpack") || strings.HasPrefix(pathItem, "/xpack"):
		dbFile = path.Join(global.CONF.Base.InstallDir, "2panel/db/xpack.db")
	case strings.HasPrefix(pathItem, "/core/enterprise") || strings.HasPrefix(pathItem, "/enterprise"):
		dbFile = path.Join(global.CONF.Base.InstallDir, "2panel/db/enterprise.db")
	case strings.HasPrefix(pathItem, "/core"):
		dbFile = path.Join(global.CONF.Base.InstallDir, "2panel/db/core.db")
	default:
		dbFile = path.Join(global.CONF.Base.InstallDir, "2panel/db/agent.db")
	}
