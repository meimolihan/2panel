...
import (
	...
	cmdUtils "github.com/2Panel-dev/2Panel/core/utils/cmd"
...
var restoreCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		if !isRoot() {
			fmt.Println(i18n.GetMsgWithMapForCmd("SudoHelper", map[string]interface{}{"cmd": "sudo 2pctl restore"}))
			return nil
		}
		...
		upgradeDir := path.Join(baseDir, "2panel", "tmp", "upgrade")
		...
		if err := files.CopyItem(false, true, path.Join(tmpPath, "2panel-agent"), "/usr/local/bin"); err != nil {
			return err
		}
		if err := files.CopyItem(false, true, path.Join(tmpPath, "2panel-core"), "/usr/local/bin"); err != nil {
			return err
		}
		if err := files.CopyItem(true, true, path.Join(tmpPath, "lang"), "/usr/local/bin"); err != nil {
			return err
		}
		if err := files.CopyItem(false, true, path.Join(tmpPath, "GeoIP.mmdb"), path.Join(baseDir, "2panel/geo")); err != nil {
			return err
		}
		_, _ = cmdUtils.NewCommandMgr().RunWithStdout("chmod", "755", "/usr/local/bin/2panel-agent", "/usr/local/bin/2panel-core")

		fmt.Println(i18n.GetMsgByKeyForCmd("RestoreStep2"))
		if err := files.CopyItem(false, true, path.Join(tmpPath, "2pctl"), "/usr/local/bin"); err != nil {
			return err
		}
		_, _ = cmdUtils.NewCommandMgr().RunWithStdout("chmod", "755", "/usr/local/bin/2pctl")
		_, _ = cmdUtils.NewCommandMgr().RunWithStdout("cp", "-r", path.Join(tmpPath, "lang"), "/usr/local/bin")
		geoPath := path.Join(global.CONF.Base.InstallDir, "2panel/geo")
		...
		if _, err := os.Stat(path.Join(tmpPath, "db")); err == nil {
			dbPath := path.Join(baseDir, "2panel")
...
func loadRollbackVersion(upgradeDir string) string {
	info, err := ctl_conf.LoadFromFile(path.Join(upgradeDir, "2pctl"), "ORIGINAL_VERSION")
	if err != nil {
		return "-"
	}
	if len(info) == 0 || info == `""` {
