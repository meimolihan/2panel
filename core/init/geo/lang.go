...
import (
	...
	fileUtils "github.com/2Panel-dev/2Panel/core/utils/files"
)
...
func initLang() {
	geoPath := path.Join(global.CONF.Base.InstallDir, "2panel/geo/GeoIP.mmdb")
	isLangExist := fileUtils.Stat("/usr/local/bin/lang/zh.sh")
	isGeoExist := fileUtils.Stat(geoPath)
	if isLangExist && isGeoExist {
		return
	}
	upgradePath := path.Join(global.CONF.Base.InstallDir, "2panel/tmp/upgrade")
