...
import (
	...
	"github.com/2Panel-dev/2Panel/core/init/migration/helper"
...
var InitSetting = &gormigrate.Migration{
	...
	Migrate: func(tx *gorm.DB) error {
		...
		_ = ctl_conf.UpdateInFile("/usr/local/bin/2pctl", "ORIGINAL_PASSWORD", "**********")
...
var InitOneDrive = &gormigrate.Migration{
	...
	Migrate: func(tx *gorm.DB) error {
		...
		if err := tx.Create(&model.BackupAccount{
			Name: "localhost",
			Type: "LOCAL",
			Vars: fmt.Sprintf("{\"dir\":\"%s\"}", path.Join(global.CONF.Base.InstallDir, "2panel/backup")),
		}).Error; err != nil {
			return err
		}
		return nil
