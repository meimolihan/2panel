...
import (
	...
	"github.com/2Panel-dev/2Panel/core/utils/firewall"
	"github.com/2Panel-dev/2Panel/core/utils/passkey"
	"github.com/2Panel-dev/2Panel/core/utils/req_helper/proxy_local"
...
func (u *SettingService) UpdateSSL(c *gin.Context, req dto.SSLUpdate) error {
	secretDir := path.Join(global.CONF.Base.InstallDir, "2panel/secret")
...
func (u *SettingService) LoadFromCert() (*dto.SSLInfo, error) {
	...
	case "import-paste", "import-local":
		...
		if _, err := os.Stat(path.Join(global.CONF.Base.InstallDir, "2panel/secret/server.crt")); err != nil {
			return nil, fmt.Errorf("load server.crt file failed, err: %v", err)
		}
		certFile, _ := os.ReadFile(path.Join(global.CONF.Base.InstallDir, "2panel/secret/server.crt"))
		data.Cert = string(certFile)

		if _, err := os.Stat(path.Join(global.CONF.Base.InstallDir, "2panel/secret/server.key")); err != nil {
			return nil, fmt.Errorf("load server.key file failed, err: %v", err)
		}
		keyFile, _ := os.ReadFile(path.Join(global.CONF.Base.InstallDir, "2panel/secret/server.key"))
...
func (u *SettingService) UpdateSystemSSL() error {
	certPath := path.Join(global.CONF.Base.InstallDir, "2panel/secret/server.crt")
	keyPath := path.Join(global.CONF.Base.InstallDir, "2panel/secret/server.key")
...
func loadInfoFromCert() (dto.SSLInfo, error) {
	...
	certFile := path.Join(global.CONF.Base.InstallDir, "2panel/secret/server.crt")
	...
	return dto.SSLInfo{
		...
		RootPath: path.Join(global.CONF.Base.InstallDir, "2panel/secret/server.crt"),
	}, nil
}

func checkCertValid() error {
	certificate, err := os.ReadFile(path.Join(global.CONF.Base.InstallDir, "2panel/secret/server.crt.tmp"))
	if err != nil {
		return err
	}
	key, err := os.ReadFile(path.Join(global.CONF.Base.InstallDir, "2panel/secret/server.key.tmp"))
	if err != nil {
		return err
	}
...
func checkProxy(req dto.ProxyUpdate) error {
	...
	defer cancel()
	request, err := http.NewRequestWithContext(ctx, http.MethodGet, "https://2panel.cn/", nil)
	if err != nil {
