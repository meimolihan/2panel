...
import (
	...
	"github.com/2Panel-dev/2Panel/core/app/dto"
	"github.com/2Panel-dev/2Panel/core/app/model"
	"github.com/2Panel-dev/2Panel/core/app/repo"
	"github.com/2Panel-dev/2Panel/core/buserr"
	...
	"github.com/2Panel-dev/2Panel/core/utils/controller"
	"github.com/2Panel-dev/2Panel/core/utils/ctl_conf"
	"github.com/2Panel-dev/2Panel/core/utils/files"
	"github.com/2Panel-dev/2Panel/core/utils/req_helper"
	upgradeUtil "github.com/2Panel-dev/2Panel/core/utils/upgrade"
	"github.com/2Panel-dev/2Panel/core/utils/xpack"
)
...
func loadServiceInfo() (serviceInfo, error) {
	...
	coreName, err := controller.LoadServiceName("2panel-core")
	if err != nil {
		global.LOG.Errorf("load core service name failed: %v", err)
		return serviceInfo{}, err
	}
	agentName, err := controller.LoadServiceName("2panel-agent")
	if err != nil {
		global.LOG.Errorf("load agent service name failed: %v", err)
		return serviceInfo{}, err
	}
	selCoreName, err := controller.SelectInitScript("2panel-core")
	if err != nil {
		global.LOG.Errorf("select core init script failed: %v", err)
		return serviceInfo{}, err
	}
	selAgentName, err := controller.SelectInitScript("2panel-agent")
	if err != nil {
...
func (u *UpgradeService) Upgrade(req dto.Upgrade) error {
	...
	baseDir := path.Join(global.CONF.Base.InstallDir, fmt.Sprintf("2panel/tmp/upgrade/%s", req.Version))
	...
	go func() {
		...
		if err := files.CopyFileWithRename(path.Join(tmpDir, "2panel-core"), "/usr/local/bin/2panel-core"); err != nil {
			global.LOG.Errorf("upgrade 1panel-core failed, err: %v", err)
			_ = settingRepo.Update("SystemStatus", "Free")
			u.handleRollback(originalDir, 1, svcInfo)
			return
		}
		if err := files.CopyFileWithRename(path.Join(tmpDir, "2panel-agent"), "/usr/local/bin/2panel-agent"); err != nil {
		...
		if err := files.CopyFileWithRename(path.Join(tmpDir, "2pctl"), "/usr/local/bin/2pctl"); err != nil {
			global.LOG.Errorf("upgrade 1pctl failed, err: %v", err)
			_ = settingRepo.Update("SystemStatus", "Free")
			u.handleRollback(originalDir, 2, svcInfo)
			return
		}
		if err := ctl_conf.UpdateInFile("/usr/local/bin/2pctl", "BASE_DIR", global.CONF.Base.InstallDir); err != nil {
			global.LOG.Errorf("upgrade basedir in 1pctl failed, err: %v", err)
			u.handleRollback(originalDir, 2, svcInfo)
			return
		}
		if err := ctl_conf.UpdateInFile("/usr/local/bin/2pctl", "LANGUAGE", oldLang); err != nil {
			global.LOG.Errorf("upgrade basedir in 1pctl failed, err: %v", err)
			u.handleRollback(originalDir, 2, svcInfo)
			return
		}
		...
		if err := files.CopyFileWithRename(path.Join(tmpDir, "GeoIP.mmdb"), path.Join(global.CONF.Base.InstallDir, "2panel/geo/GeoIP.mmdb")); err != nil {
...
func (u *UpgradeService) LoadRelease() ([]dto.ReleasesNotes, error) {
	...
	url := "https://2panel.cn/docs/v2/search/search_index.json"
	useIntlDocs := false
	...
	if useIntlDocs {
		url = "https://docs.2panel.pro/v2/search/search_index.json"
	}
...
func (u *UpgradeService) handleBackup(originalDir string, svcInfo serviceInfo) error {
	if err := files.CopyItem(false, true, "/usr/local/bin/2panel-core", originalDir); err != nil {
		return err
	}
	if err := files.CopyItem(false, true, "/usr/local/bin/2panel-agent", originalDir); err != nil {
		return err
	}
	if err := files.CopyItem(false, true, "/usr/local/bin/2pctl", originalDir); err != nil {
		return err
	}
	...
	if err := files.CopyItem(true, true, path.Join(global.CONF.Base.InstallDir, "2panel/db"), originalDir); err != nil {
		return err
	}
	if err := files.CopyItem(false, true, path.Join(global.CONF.Base.InstallDir, "2panel/geo/GeoIP.mmdb"), originalDir); err != nil {
		return err
	}
	return nil
}
...
func (u *UpgradeService) handleRollback(originalDir string, errStep int, svcInfo serviceInfo) {
	_ = settingRepo.Update("SystemStatus", "Free")
	dbPath := path.Join(global.CONF.Base.InstallDir, "2panel")
	...
	if err := files.CopyFileWithRename(path.Join(originalDir, "2panel-core"), "/usr/local/bin/2panel-core"); err != nil {
		global.LOG.Errorf("rollback 1panel-core failed, err: %v", err)
	}
	if err := files.CopyFileWithRename(path.Join(originalDir, "2panel-agent"), "/usr/local/bin/2panel-agent"); err != nil {
		global.LOG.Errorf("rollback 1panel-agent failed, err: %v", err)
	}
	if errStep == 1 {
		return
	}
	if err := files.CopyFileWithRename(path.Join(originalDir, "2pctl"), "/usr/local/bin/2pctl"); err != nil {
	...
	if err := files.CopyFileWithRename(path.Join(originalDir, "GeoIP.mmdb"), path.Join(global.CONF.Base.InstallDir, "2panel/geo/GeoIP.mmdb")); err != nil {
