...
import (
	...
	"github.com/2Panel-dev/2Panel/core/utils/controller/manager"
	"github.com/2Panel-dev/2Panel/core/utils/ssh"
)
...
func RestartPanel(core, agent, reload bool) {
	...
	if reload {
		if err := client.Reload(); err != nil {
			global.LOG.Errorf("restart 2panel service failed, err: %v", err)
			return
		}
	}
	if agent {
		if err := client.Operate("restart", "2panel-agent"); err != nil {
			global.LOG.Errorf("restart 1panel agent service failed, err: %v", err)
			return
		}
	}
	if core {
		if err := client.Operate("restart", "2panel-core"); err != nil {
...
func loadFromPredefined(mgr Controller, keyword string) string {
	predefinedMap := map[string][]string{
		...
		"2panel-core":  {"2panel-core.service"},
		"2panel-agent": {"2panel-agent.service"},
